...
Cloud cost visibility, budget risk monitoring, governance control, and optimization intelligence in one workspace.
""")
st.write(
    "Analyze AWS cloud spend, identify ownership gaps, detect cost risks, "
    "and generate FinOps-ready insights."
)

data_source = st.radio(
    "Select Data Source",
    ["Upload CSV", "AWS Cost Explorer"],
    horizontal=True
)
...
with tabs[0]:
    ...
    with st.container(border=True):
        ...
        with st.container(border=True):
            ...
            st.caption("Estimates how fast the cloud budget is being consumed.")
...
with tabs[1]:
    ...
    with st.container(border=True):
        st.subheader("FinOps Governance Score")
...
with tabs[2]:
    ...
    with st.container(border=True):
        ...
        st.caption("Tracks recommended FinOps remediation actions, owners, severity, and estimated savings.")
...
with tabs[3]:
    ...
    with st.container(border=True):
        st.subheader("Advanced Anomaly Detection Engine")
